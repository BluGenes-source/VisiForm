> Phase 118 note (2026-06-27): Phase 118 Callback Corner Menu and Status Geometry implementation is ready for developer validation at version `1.0.24`, updated from Phase 117 version `1.0.23`. Phase 117 Preview Mode widget interaction remains tabled and pending developer validation; Phase 118 does not modify Preview Mode interaction behavior. Design Mode now shows selected-widget model-space geometry in a dedicated status-bar segment for exactly one non-root visual selection. The selected widget's upper-right resize handle has a subtle callback indicator and opens an editor-only popup on click, listing assigned callbacks from existing `WidgetRegistry` event metadata and widget event properties or an empty state. Dragging beyond the existing threshold preserves normal upper-right resize behavior. Popup actions switch the Property Inspector to Events and reveal/highlight the callback row without executing callbacks, dirtying the project, serializing state, or changing assignments. Focused static validation passed; the Windows Debug build and manual runtime checklist remain pending. See `docs/agent_plans/phase_118_callback_corner_menu_plan.md`.

> Phase 117 note (2026-06-25): Phase 117 Preview Mode Widget Interaction implementation is ready for developer validation at version `1.0.23`, updated from Phase 116 version `1.0.22`. Editor Preview Mode now keeps temporary Button, Text Box, Check Box, Radio Button, Combo Box, List Box, Slider, Scroll Bar, Tab Control, and Table/Grid interaction state inside editor-owned UI state instead of `WidgetNode`, including focus/hover/press, checked/selected state, selected indexes, active tabs, temporary Table/Grid selected row/column, temporary Text Box text, and temporary numeric range values. Preview Text Boxes reuse the existing multiline/word-wrap `TextEditControl` path and discard edits on Preview exit; read-only Text Boxes block mutation while preserving focus/selection behavior. Preview Combo Boxes now open the existing `DropdownControl` popup and store only a temporary selected index, and Preview Table/Grid clicks update only a temporary selected row/column pair matching the current runtime model. Preview clicks and drags update supported controls without design selection, drag, resize, creation, dirty state, undo history, autosave/recovery, `.vfb.json`, generated event handlers, or generated output changes. Focused static validation passed; the Windows Debug build, Catch2 execution, manual runtime checklist, generated-project build, and autosave/recovery checks remain pending. See `docs/agent_plans/phase_117_preview_mode_widget_interaction_plan.md`.

> Phase 116 note (2026-06-25): Phase 116 Multiline Text Box Editing implementation is ready for developer validation at version `1.0.22`, updated from Phase 115 version `1.0.21`. The existing Text Box widget now opts into multiline Text property editing when `Multiline` is resolved true, preserving newline content, supporting Enter insertion, measured-font caret navigation, Shift selection, mouse drag selection, word-wrap-aware visual lines, and editor-only scroll offsets while preserving single-line defaults. Text Box `Multiline` and `Word Wrap` now appear as direct Text Box Behavior Boolean rows and commit through the direct sparse text-layout override path instead of the generic property map. Generated Text Box output now preserves multiline pasted newlines and Enter inserts newlines when generated runtime `multiline` is true. Editor Preview Mode renders multiline Text Boxes through the existing layout path but does not commit temporary text edits into the project model. Focused static validation passed; the Windows Debug build, Catch2 execution, manual runtime checklist, generated-project build, and autosave/recovery checks remain pending. See `docs/agent_plans/phase_116_multiline_text_box_editing_plan.md`.

> Phase 115 note (2026-06-25): Phase 115 Text Layout and Wrapping Foundation implementation is ready for developer validation at version `1.0.21`, updated from Phase 114 version `1.0.20`. Sparse widget text-layout overrides now cover `multiline`, `wordWrap`, and `overflowMode`; old projects default to single-line, no-wrap, clipped text. A shared editor text-layout helper uses `visage::Font` metrics for explicit newlines, word wrapping, oversized-word fallback, alignment, clipping, and single-line ellipsis, and Design/Preview rendering consumes it for Label, Button, Text Box, Check Box, Radio Button, Group Box, Frame, tab labels, and Status Bar. The Property Inspector adds Text Layout rows, serialization stores only explicit overrides, invalid overflow modes fall back to `Clip`, and generated runtime output emits equivalent layout intent and helper code for the supported widgets. Text Box multiline rendering is foundational only; full multiline editing and multiline final-line ellipsis remain future work. Focused static validation passed and serialization/resolver coverage was added; the Windows Debug build, Catch2 execution, generated-project build, autosave/recovery checks, and manual runtime checklist remain pending. See `docs/agent_plans/phase_115_text_layout_wrapping_plan.md`.

> Phase 114 note (2026-06-24): Phase 114 Typography and Text Rendering Foundation implementation is ready for developer validation at version `1.0.20`, updated from Phase 113 version `1.0.19`. Sparse project typography overrides now cover font family, size, weight, italic, text padding, and disabled-text treatment; sparse widget typography overrides cover family, size, weight, italic, horizontal alignment, meaningful vertical alignment, and text padding for the initial text-bearing widget scope. Resolution follows Look and Feel typography, project overrides, then widget overrides. The Property Inspector adds compact Typography rows inside the existing Appearance area, including readable Font Weight labels (`Regular` / `Bold`) backed by numeric internal values (`400` / `700`) and a `Regular` fallback for unsupported numeric weights. Design/Preview rendering consumes resolved typography for supported text widgets, serialization stores only explicit overrides, custom Look and Feel presets preserve typography, and generated output emits equivalent typography style intent without editor-only font resources. Focused static validation passed and serialization/resolver tests were added; the Windows Debug build, Catch2 execution, generated-project build, and manual runtime checklist remain pending. See `docs/agent_plans/phase_114_typography_text_rendering_plan.md`.

> Phase 113 note (2026-06-20): Phase 113 Appearance Controls and State Preview implementation is ready for developer validation at version `1.0.19`, updated from Phase 112 version `1.0.18`. Normal widget Corner Radius and Border Thickness now recover registry-backed `0-25` sliders, preserving zero semantics, immediate repaint, and one no-op-safe undo command per completed drag. The Appearance section adds one editor-only `Preview State` toggle that renders only the selected widget through the shared state resolver while keeping Design Mode overlays visible; it never mutates, dirties, serializes, generates, or adds undo history and clears on selection/context changes, resets, and Preview Mode entry. Focused static validation passed and registry metadata coverage was added; the Windows Debug build, Catch2 execution, and manual runtime checklist remain pending.

> Phase 112 note (2026-06-20): Phase 112 Widget State Appearance Overrides implementation is ready for developer validation. Version `1.0.18` adds sparse per-widget overrides for control surface, text, border, accent, highlight edge, shadow edge, and focus outline colors across compatible Normal, Hover, Pressed, Focused, Checked/Selected, and Disabled states. Resolution remains preset, project overrides, normal widget overrides, then widget state overrides; disabled, pressed, checked/selected, hover, and normal retain Phase 107 priority with focus as an overlay. The existing Property Inspector Appearance section now selects states, displays inherited normal values, and provides undoable Reset Property, Reset State, and Reset All Appearance actions. Design Mode remains normal-only, while Preview Mode and generated output consume shared state intent. Sparse serialization, value-copy paths, focused tests, and static validation are complete; the Windows Debug build, Catch2 execution, generated-project build, and manual runtime checklist remain pending.

> Phase 111 note (2026-06-20): Phase 111 Per-Widget Look and Feel Overrides implementation is ready for developer validation. Version `1.0.17` adds one sparse widget-owned override structure for seven colors and three metrics, resolved in the order built-in/custom preset, project overrides, then widget overrides. The Property Inspector exposes a focused Appearance section for the fourteen supported widget types, displays inherited values, disables multi-selection editing, and provides undoable reset-one/reset-all operations. Design Mode, Preview Mode, serialization/autosave/recovery, value-copy duplication/clipboard paths, and generated output consume the shared final resolver. Focused static validation passed and focused tests were added; the Windows Debug build, Catch2 execution, generated-project build, and manual runtime checklist remain pending.

> Phase 110 note (2026-06-20): Phase 110 Look and Feel Preset Management implementation is ready for developer validation. One versioned user-data JSON library now supplies reusable custom presets with stable opaque identifiers and complete normalized styles. The existing Look and Feel editor exposes Save New, Duplicate, Rename, Delete, Import, and Export; built-ins remain protected, active-preset deletion explicitly falls back to `VisiFormDark` while preserving overrides, unavailable project presets retain their stored identifier while rendering through the same fallback, and generated output receives resolved values without a local-library dependency. The phase version is `1.0.16`, updated from Phase 109 version `1.0.15`. Focused static validation passed and focused tests were added; the Windows Debug build, Catch2 execution, generated-project build, and manual runtime/persistence checklist remain pending.

> Phase 109 note (2026-06-19): Phase 109 Look and Feel Editor Baseline implementation is ready for developer validation. `Project > Edit Look and Feel...` now edits ten shared colors and five compact metrics as sparse project overrides layered over the existing built-in `lookAndFeelId`; built-in presets remain immutable, base-preset changes preserve overrides, and empty overrides are omitted from `.vfb.json`. Apply and OK are no-op-safe document-state commands, Cancel discards only unapplied values, Reset to Preset remains temporary until committed, and the dialog includes a representative live sample using temporary values. Design Mode, Preview Mode, editor splitters, autosave/recovery serialization, and generated output consume the final resolved style. The phase version is `1.0.15`, updated from Phase 108 version `1.0.14`. Focused static validation passed and focused tests were added; the Windows Debug build, Catch2 execution, generated-project build, and manual runtime checklist remain pending.

> Phase 108 note (2026-06-19): Phase 108 Look and Feel Styling Integration implementation is ready for developer validation. The existing project-level `lookAndFeelId` remains the source of truth, and the four existing presets now resolve through one registry-owned style containing application/control/recessed/raised surfaces, text roles, state colors, bevel edges, and compact metrics. Design Mode, Preview Mode, editor splitters, and generated runtime styling consume the shared resolved values while editor overlays remain independent. Missing and unknown identifiers safely fall back to `VisiFormDark`; `.vfb.json` still stores only the existing identifier and widget overrides. The phase version is `1.0.14`, updated from Phase 107 version `1.0.13`. Focused static validation passed and focused tests were added; the Windows Debug build, Catch2 execution, generated-project build, DPI/zoom checks, and manual runtime checklist remain pending.
