...
void TextEditControl::begin(std::string text, bool selectAllText, bool multiline)
{
    originalText_ = text;
    text_ = std::move(text);
    editing_ = true;
...
bool TextEditControl::keyPress(const visage::KeyEvent& event)
{
    ...
    case KeyCode::Right:
        ...
        return true;
    case KeyCode::Home:
        moveCursor(multiline_ ? lineStartForIndex(cursorIndex_) : 0);
        return true;
    case KeyCode::End:
        moveCursor(multiline_ ? lineEndForIndex(cursorIndex_) : text_.size());
        return true;
    case KeyCode::Return:
        if (multiline_) {
            insertText("\n");
    ...
    case KeyCode::Up:
        if (multiline_) {
            ...
            if (lineIndex > 0) {
                moveCursor(indexForLineColumn(lineIndex - 1, columnIndex));
            }
            return true;
        }
        return false;
...
void TextEditControl::draw(visage::Canvas& canvas, const visage::Font& font, bool drawText) const
{
    ...
    if (!multiline_ && hasSelection() && selectionWidth > 0.0f) {
