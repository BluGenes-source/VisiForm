...
std::string emitGeneratedBaseCpp(const visiform::model::ProjectDocument& document)
{
    ...
    stream << "            drawVisualBevel(canvas, tabX, y, tabWidth, kTabHeaderHeight, widget, tabFill, false);\n";
    stream << "            if (drawText) {\n";
    stream << "                canvas.setColor(canvasColor(visualTextColor(widget)));\n";
    ...
    stream << "        const RuntimeColor well = visualStateFill(widget, blendColor(widget.style.fillColor, makeColor(0x00, 0x00, 0x00), 0.12f), widget.interaction.pressed);\n";
