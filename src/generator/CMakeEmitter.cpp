...
std::string CMakeEmitter::emitCMakeLists(const model::ProjectDocument& document, const utils::AppSettings& settings) const
{
    ...
    stream << "set(_VISIFORM_VISAGE_SOURCE_CANDIDATES)\n";
    stream << "if (VISIFORM_VISAGE_SOURCE_DIR)\n";
    stream << "    list(APPEND _VISIFORM_VISAGE_SOURCE_CANDIDATES \"${VISIFORM_VISAGE_SOURCE_DIR}\")\n";
    stream << "endif()\n";
    stream << "if (DEFINED ENV{VISIFORM_VISAGE_SOURCE_DIR} AND NOT \"$ENV{VISIFORM_VISAGE_SOURCE_DIR}\" STREQUAL \"\")\n";
    stream << "    list(APPEND _VISIFORM_VISAGE_SOURCE_CANDIDATES \"$ENV{VISIFORM_VISAGE_SOURCE_DIR}\")\n";
    stream << "endif()\n";
    stream << "list(APPEND _VISIFORM_VISAGE_SOURCE_CANDIDATES\n";
    stream << "    \"${CMAKE_CURRENT_LIST_DIR}/../visage\"\n";
    stream << "    \"${CMAKE_CURRENT_LIST_DIR}/../../visage\"\n";
    stream << "    \"${CMAKE_CURRENT_LIST_DIR}/../../../visage\"\n";
    stream << ")\n\n";
    stream << "set(_VISIFORM_RESOLVED_VISAGE_SOURCE_DIR \"\")\n";
    stream << "foreach (_VISIFORM_VISAGE_SOURCE_CANDIDATE IN LISTS _VISIFORM_VISAGE_SOURCE_CANDIDATES)\n";
    stream << "    if (EXISTS \"${_VISIFORM_VISAGE_SOURCE_CANDIDATE}/CMakeLists.txt\")\n";
    stream << "        get_filename_component(\n";
    stream << "            _VISIFORM_RESOLVED_VISAGE_SOURCE_DIR\n";
    stream << "            \"${_VISIFORM_VISAGE_SOURCE_CANDIDATE}\"\n";
    stream << "            ABSOLUTE\n";
    stream << "        )\n";
    stream << "        break()\n";
    stream << "    endif()\n";
    stream << "endforeach()\n\n";
    stream << "if (_VISIFORM_RESOLVED_VISAGE_SOURCE_DIR)\n";
    stream << "    message(STATUS \"Using local Visage source: ${_VISIFORM_RESOLVED_VISAGE_SOURCE_DIR}\")\n\n";
...
std::string CMakeEmitter::emitReadme(const model::ProjectDocument& document, const utils::AppSettings& settings) const
{
    ...
    stream << "CMake first checks `VISIFORM_VISAGE_SOURCE_DIR`, then the `VISIFORM_VISAGE_SOURCE_DIR` environment variable, then nearby `visage` sibling folders such as `../visage`, `../../visage`, and `../../../visage`.\n\n";
    stream << "Set `VISIFORM_VISAGE_SOURCE_DIR` to a local Visage checkout when you want to force a specific path, for example `C:/dev/visage` on Windows or `$HOME/dev/visage` on macOS/Linux.\n\n";
    if (!localVisageSourceDirectory.empty()) {
    ...
    stream << "If no configured, environment, or nearby local source path points to a valid Visage source tree, CMake falls back to `FetchContent` using the configured repository and tag values.\n\n";
